...
class UploadDriverInfo(View):
    ...
    def post(request):
        ...
        try:
            ...
            return HttpResponse(response.respond(), content_type="application/json")
        try:
            if consts.PARAM_DRIVER_LICENSE_IMAGE in request.FILES:
                dl_image = request.FILES[consts.PARAM_DRIVER_LICENSE_IMAGE]
            else:
                dl_image = None

            if consts.PARAM_DRIVER_NATIONAL_ID_IMAGE in request.FILES:
                nid_image = request.FILES[consts.PARAM_DRIVER_NATIONAL_ID_IMAGE]
            else:
                nid_image = None
            driver = DriverModel(user=user,
                                 national_id=request.POST[consts.PARAM_DRIVER_NATIONAL_ID],
                                 national_id_image=nid_image,
                                 driving_license=request.POST[consts.PARAM_DRIVER_LICENSE],
                                 driving_license_image=dl_image)
...
class UploadVehicleInfo(View):
    ...
    def post(request):
        ...
        try:
            ...
            return HttpResponse(response.respond(), content_type="application/json")
        try:
            if consts.PARAM_VEHICLE_REG_PHOTO in request.FILES:
                reg_image = request.FILES[consts.PARAM_VEHICLE_REG_PHOTO]
            else:
                reg_image = None
            driver = DriverModel.objects.get(user=user)
            ...
            new_vehicle = VehicleModel(registration_number=request.POST[consts.PARAM_VEHICLE_NUMBER],
                                       ...
                                       registration_certificate_image=reg_image,
                                       driver=driver)
            new_vehicle.save()
            response = common_response.CommonResponse(success=True,
